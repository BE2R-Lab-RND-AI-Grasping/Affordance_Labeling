    ...
    for elevation in elevation_samples:
        ...
        if elevation == 0 or elevation == 180:
            for rot in rotation_samples:
                get_render([0, 0, np.cos(np.radians(elevation))],
                           rot, mesh, render_dir)
        else:
            for azimuth in azimuth_samples:
                for rot in rotation_samples:
                    get_render([np.cos(np.radians(azimuth))*np.sin(np.radians(elevation)), np.sin(np.radians(
                        azimuth))*np.sin(np.radians(elevation)), np.cos(np.radians(elevation))], rot, mesh, render_dir)

def get_best_render_match(render_dir, scene_results):
    ...
    return path_list[max_ind], max_bbox, cropped, max_mask, max_score
